...
// Based on the the tl_build_config.h header from the Tiny Lib project.
// Please consider submitting improvements and fixes there.
//
// The Tiny Lib project is
//
...
// ==========================
//
// An operating system is detected is defined as an `OS_<NAME>` symbols.
// For example, on Windows the `OS_WIN` is defined to 1, and all the other
...
//
// The aggregate COMPILER_MINGW is defined when the compiler is wither MINGW32
// or MINGW64.
//
//
// C++ standard support detection
// ==============================
//
// A supported set of STL standards is detected. This information is available
// via `COMPILER_SUPPORTS_CXX<year>` symbols.
//
//
// CPU detection
// =============
...
//
// There are some family-specific defines which allows to tell CPUs apart within
// the family. Those defines are easier to follow when looking at the actual
// definition (otherwise it will just become too much of copy-paste to show all
// of the available permutations).
//
//
// Version history
// ===============
...
#  define OS_ZOS 1
#else
// Generic operation system: either unknown, or the code targets bare-metal
// embedded device without operation system.
#  define OS_GENERIC 1
#endif
...
// more specific macro.
#if OS_AIX || OS_ANDROID || OS_ASMJS || OS_FREEBSD || OS_LINUX || OS_MAC ||    \
...
#endif
// C++20 check.
// NOTE: The 201709 is what gcc-11 defines for -std=c++20
...
#endif
// C++23 check.
#if (defined(__cplusplus) && (__cplusplus > 202211L))
#  define COMPILER_SUPPORTS_CXX23 1
#else
#  define COMPILER_SUPPORTS_CXX23 0
#endif
...
// Test whether TEST indicates a presence of a configuration. The presence is
// defines as:
//   - Defined non-zero symbol (#define TEST 1)
//   - Defined empty preprocessor variable (#define TEST)
//
// If TEST is 0 then the expression result is 0.
#define _TL_BUILD_CONFIG_CAN_USE(TEST)                                         \
  ((((TEST + 0 != -14) && (7 - TEST - 7 == 14))) || ((0 + TEST - 0) != 0))

#if ARCH_CPU_X86_FAMILY
// SSE 2.
#  if defined(__SSE2__) && _TL_BUILD_CONFIG_CAN_USE(__SSE2__)
#    define ISA_CPU_X86_SSE2 1
#  elif ARCH_CPU_64_BITS
// 64-bit x86 CPUs always have SSE2 available
#    define ISA_CPU_X86_SSE2 1
#  elif defined(_M_IX86_FP) && _M_IX86_FP == 2
// 32-bit mode SSE2.
// https://stackoverflow.com/a/18570487
#    define ISA_CPU_X86_SSE2 1
#  else
#    define ISA_CPU_X86_SSE2 0
#  endif

// SSE 3.
#  if defined(__SSE3__) && _TL_BUILD_CONFIG_CAN_USE(__SSE3__)
#    define ISA_CPU_X86_SSE3 1
#  else
#    define ISA_CPU_X86_SSE3 0
#  endif

// SSE 4.1.
#  if defined(__SSE4_1__) && _TL_BUILD_CONFIG_CAN_USE(__SSE4_1__)
#    define ISA_CPU_X86_SSE4_1 1
#  else
#    define ISA_CPU_X86_SSE4_1 0
#  endif

// AVX.
#  if defined(__AVX__) && _TL_BUILD_CONFIG_CAN_USE(__AVX__)
#    define ISA_CPU_X86_AVX 1
#  else
#    define ISA_CPU_X86_AVX 0
#  endif

// AVX 2.
#  if defined(__AVX2__) && _TL_BUILD_CONFIG_CAN_USE(__AVX2__)
#    define ISA_CPU_X86_AVX2 1
#  else
#    define ISA_CPU_X86_AVX2 0
#  endif

// FMA.
#  if defined(__FMA__) && _TL_BUILD_CONFIG_CAN_USE(__FMA__)
#    define ISA_CPU_X86_FMA 1
#  else
#    define ISA_CPU_X86_FMA 0
#  endif
#endif

#if ARCH_CPU_ARM_FAMILY
#  define ISA_CPU_ARM_V8 (__ARM_ARCH >= 8)

#  if defined(__ARM_NEON) && _TL_BUILD_CONFIG_CAN_USE(__ARM_NEON)
#    define ISA_CPU_ARM_NEON 1
#  else
#    define ISA_CPU_ARM_NEON 0
#  endif
#else
#  define ISA_CPU_ARM_NEON 0
#  define ISA_CPU_ARM_V8 0
#endif
