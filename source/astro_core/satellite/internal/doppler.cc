...
auto CalculateDopplerFactor(const ITRF& site, const ITRF& satellite) -> double {
  ...
  const Vec3 direction = Vec3(satellite.position.GetCartesian()) -
                         Vec3(site.position.GetCartesian());
  ...
  const double v_site =
      Vec3(site.velocity.GetCartesianOr({0, 0, 0})).Dot(normalized_direction);
  const double v_satellite = Vec3(satellite.velocity.GetCartesianOr({0, 0, 0}))
